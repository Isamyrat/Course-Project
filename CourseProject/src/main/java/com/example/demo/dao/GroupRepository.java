...
public interface GroupRepository extends JpaRepository<Group, Long> {
    ...
    @Query("select g from  Group g where  g.user_teacher.id = :id and (g.status = :status or g.status = :status1)")
    List<Group> findByTeacherAndStatus(Long id, Status status, Status status1);
